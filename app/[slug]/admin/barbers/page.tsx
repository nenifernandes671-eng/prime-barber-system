...
export default function PublicBarbeiros() {
  ...
  const { data, error } = await supabase
    .from('barbeiros')
    .select('*')
    .eq('tenant_id', tenantId)   // ← filtra pelo tenant certo
    .eq('ativo', true)            // ← só mostra ativos pro cliente
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/admin/criar-barbeiro', {
        ...
        body: JSON.stringify({ nome: nome.trim(), email: email.trim(), senha, telefone: telefone.trim() }),
      })
